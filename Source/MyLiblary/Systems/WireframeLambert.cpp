...
#include "Liblary/Systems/WireframeLambert.h"
...
WireLambertShader::WireLambertShader(ID3D11Device* device) :Shader(device)
{
	// 頂点シェーダー
	{
		...
		hr = device->CreateInputLayout(element_desc,
			ARRAYSIZE(element_desc), csoData.get(), csoSize, input_layout.GetAddressOf());
		_ASSERT_EXPR(SUCCEEDED(hr), HResultTrace(hr));
	}
...
void WireLambertShader::Draw(ID3D11DeviceContext* context, const Model* model)
{
	...
	for (const ModelResource::Mesh& mesh : resource->GetMeshes())
	{
		...
		::memset(&constant_buffer_mesh, 0, sizeof(constant_buffer_mesh));
