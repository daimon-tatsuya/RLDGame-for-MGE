...
GeometricPrimitive::GeometricPrimitive(ID3D11Device* device)
{
	...
	// create rasterizer state : solid mode //線描画
	{
	...
	// create rasterizer state : wireframe mode //塗りつぶし描画
	{
		D3D11_RASTERIZER_DESC rasterizer_desc = {};
	...
	// create depth stencil state
	{
		...
		depth_stencil_desc.DepthEnable = TRUE;     //深度テストの真偽        depth_stencil_desc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL; //深度ステンシルバッファーの書き込みオンかオフか
		depth_stencil_desc.DepthFunc = D3D11_COMPARISON_LESS; //深度データを既存の深度データと比較する //今回はソースデータが宛先データよりも小さい場合、比較は成功します。
		depth_stencil_desc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
		depth_stencil_desc.StencilEnable = FALSE;  //ステンシルテストの真偽
		depth_stencil_desc.StencilReadMask = 0xFF; //ステンシルデータ読み取り
		depth_stencil_desc.StencilWriteMask = 0xFF;//ステンシルデータ書き取り
	...
	// create constant buffer
	{
		...
		buffer_desc.Usage = D3D11_USAGE_DEFAULT; //読み取り書き取り使用するもの予想//今回はGPUで読み取り、書き込みできる
		buffer_desc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;//3dのためコンスタントを使用
	...
	//vertex vertices[24] = {}; //立法体の面×頂点の数 //面の頂点
	//u_int indices[36] = {};   //立法体の面を三角形で分けたときの頂点×立方体の面 //面を三角形で二つにしたときの頂点
	...
	//// |   -y   |
	//// 2-------3
	//face = 0;//値の変えて入れる頂点位置をずらす

	//vertices[face * 4 + 0].position = DirectX::XMFLOAT3(-0.5f, +0.5f, +0.5f);     //立方体の頂点に値を入れてる
	...
	//vertices[face * 4 + 0].normal = DirectX::XMFLOAT3(+0.0f, +1.0f, +0.0f);     //立方体の法線に値を入れてる
	...
	//indices[face * 6 + 0] = face * 4 + 0;       //三角形の頂点
...
void GeometricPrimitive::CreateBuffers(ID3D11Device* device, Vertex* vertices, int numVertices, u_int* indices, int numIndices)
{
	HRESULT hr;
	{
		...
		buffer_desc.Usage = D3D11_USAGE_IMMUTABLE; // 読み取り書き取り使用するもの予想//今回はGPUのみ読み取れる、書き込みはできない
		buffer_desc.BindFlags = D3D11_BIND_VERTEX_BUFFER; //頂点バッファーを入力アンセンブラーステージにバインド
		buffer_desc.CPUAccessFlags = 0;//読み取りか、書き取りか使わんければ０
		buffer_desc.MiscFlags = 0;
		buffer_desc.StructureByteStride = 0;
		subresource_data.pSysMem = vertices; //初期化データへのポインタ。
		subresource_data.SysMemPitch = 0; //Not use for vertex buffers.mm  //テクスチャの1つの行の先頭から次の行までの距離（バイト単位）。
		subresource_data.SysMemSlicePitch = 0; //Not use for vertex buffers. //ある深度レベルの先頭から次の深度レベルまでの距離（バイト単位）。
	...
	bool  wireframe)
{
	...
	immediateContext->UpdateSubresource(constant_buffer.Get(), 0, 0, &data, 0, 0); //CPUは、メモリからマッピング不可能なメモリに作成されたサブリソースにデータをコピーします。//(宛名リソースへのポインター、ゼロベースインデックス、リソースデータをコピーする宛先サブリソースの部分を定義するボックスへのポインタ-,メモリ内のソースデータへのポインタ,ソースデータの1行のサイズ,ソースデータの1つの深度スライスのサイズ)
	immediateContext->VSSetConstantBuffers(0, 1, constant_buffer.GetAddressOf()); //頂点シェーダーパイプラインステージで使用される定数バッファーを設定します。

	u_int stride = sizeof(Vertex);
	u_int offset = 0;
	immediateContext->IASetVertexBuffers(0, 1, vertex_buffer.GetAddressOf(), &stride, &offset); //頂点バッファーの配列を入力アセンブラーステージにバインドします。
	immediateContext->IASetIndexBuffer(index_buffer.Get(), DXGI_FORMAT_R32_UINT, 0); //インデックスバッファを入力アセンブラステージにバインドします。
	immediateContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);  //プリミティブタイプに関するバインド情報、および入力アセンブラステージの入力データを記述するデータ順序。
	immediateContext->IASetInputLayout(input_layout.Get()); //input-layoutオブジェクトをinput-assemblerステージにバインドします。

	immediateContext->VSSetShader(vertex_shader.Get(), nullptr, 0); //頂点シェーダーをデバイスに設定します。
	immediateContext->PSSetShader(pixel_shader.Get(), nullptr, 0); //ピクセルシェーダーをデバイスに設定します

	immediateContext->OMSetDepthStencilState(depth_stencil_state.Get(), 1); //出力マージステージの深度ステンシル状態を設定します。
	if (wireframe)
	{
		immediateContext->RSSetState(rasterizer_states[1].Get());  //パイプラインのラスタライザステージのラスタライザ状態を設定します。
	}
	else
	{
		immediateContext->RSSetState(rasterizer_states[0].Get());  //パイプラインのラスタライザステージのラスタライザ状態を設定します。
	}

	D3D11_BUFFER_DESC buffer_desc;
	index_buffer->GetDesc(&buffer_desc);//バッファリソースのプロパティを取得します。
	immediateContext->DrawIndexed(buffer_desc.ByteWidth / sizeof(u_int), 0, 0);  //インデックス付きのインスタンス化されていないプリミティブを描画します。
}
...
GeometricSphere::GeometricSphere(ID3D11Device* device, u_int slices, u_int stacks) : GeometricPrimitive(device)
{
	...
	// This is just skipping the top pole vertex.//球全体面
	u_int base_index = 1;
