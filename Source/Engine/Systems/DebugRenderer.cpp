...
void DebugRenderer::Render(ID3D11DeviceContext* deviceContext, const DirectX::XMFLOAT4X4& view, const DirectX::XMFLOAT4X4& projection)
{
	//シェーダー設定
	...
	//球描画
	deviceContext->IASetVertexBuffers(0, 1, sphere_vertex_buffer.GetAddressOf(), &stride, &offset);
	for (const Sphere& sphere : spheres)
	{
		...
		deviceContext->UpdateSubresource(constant_buffer.Get(), 0, 0, &cbMesh, 0, 0);
		deviceContext->Draw(sphere_vertex_count, 0);
	}
	spheres.clear();

	//円柱描画
	deviceContext->IASetVertexBuffers(0, 1, cylinder_vertex_buffer.GetAddressOf(), &stride, &offset);
	for (const Cylinder& cylinder : cylinders)
	{
		...
		deviceContext->UpdateSubresource(constant_buffer.Get(), 0, 0, &cbMesh, 0, 0);
		deviceContext->Draw(cylinder_vertex_count, 0);
	}
	cylinders.clear();
}
